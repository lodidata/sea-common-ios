...
class RewardView: UIView {
    ...
    //指向尖头
    private lazy var directImg: UIImageView = {
        let img = UIImageView.init(image: UIImage.init(named: "btn-pin"))
        img.contentMode = .center
        return img
    }()
    // 中间按钮
    private lazy var centerImg: UIImageView = {
        let img = UIImageView.init(image: UIImage.init(named: "btn-center"))
        img.contentMode = .center
        return img
    }()
    // 背景转盘
    ...
    private lazy var backImageView: UIImageView = {
        let img = UIImageView.init()
        img.image = UIImage.init(named: "background-2")
        img.contentMode = .center
        return img
    }()
    // 上次转动角度
    ...
    private func setupLayout() {
        ...
        directImg.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            make.top.equalTo(25)
        }
