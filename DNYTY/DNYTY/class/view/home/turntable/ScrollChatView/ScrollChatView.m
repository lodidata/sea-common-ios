...
- (void)action{
    ...
    if (self.scrollIndex >= self.dataList.count) {
        self.scrollIndex = 0;
    }
    ScrollChatTextModel *md = [self.yxDataSource objectAtIndex:self.scrollIndex];
...
//    static NSString *cellIdentifier = @"cellIdentifier";
    ...
    WLAwardMsgCell *cell = (WLAwardMsgCell *)[tableView dequeueReusableCellWithIdentifier:@"cellIdentifier"];
    cell.textLab.numberOfLines = 0;
    cell.textLab.transform = CGAffineTransformMakeScale(1, -1);
//    if (!cell)
//    {
...
//    cell.textLabel.text = md.yx_content;
    cell.textLab.text = md.yx_content;
    
    if (self.font) {
        cell.textLab.font = self.font;
    }
    if (self.color) {
        cell.textLab.textColor = self.color;
    }
...
- (void)setupUI {
    ...
    [self registerClass:[WLAwardMsgCell class] forCellReuseIdentifier:@"cellIdentifier"];
}
