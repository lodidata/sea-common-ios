...
//    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.register(NoticeCenterCell.self, forCellReuseIdentifier: "notice")
    ...
    override func viewDidLoad() {
        ...
        navView.titleLab.text = "notice0".wlLocalized
        setMj_header_footer()
        //getNoticeList()
    }
...
extension WLNoticesController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataList?.count ?? 0
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "notice") as! NoticeCenterCell
...
//            cell.setDataModel(dataModel: dataList?[indexPath.row], isSelect: selectList[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let detailVc = WLNoticeDetailController()
        if indexPath.row < dataList?.count ?? 0 {
...
extension WLNoticesController {
    func getNoticeList() {
        WLProvider.request(.noticeList(page: page, page_size: 10)) { result in
            switch result {
            case let .success(response):
                let json = JSON(response.data)
                if let baseModel = WLBaseModel(JSON: json.dictionaryObject ?? [:]) {
                    if baseModel.state == 0 {
                        if let data: [NoticeModel] = Array(JSONString: json["data"].rawString() ?? "") {
                            if self.page == 1 {
                                self.dataList?.removeAll()
